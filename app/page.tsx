      ...
      <section className="grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
        <div className="space-y-8">
          ...
          <div className="flex flex-wrap gap-4">
            <Button onClick={() => router.push("/sign-up")}>{t("ctaStartFree")}</Button>
        ...
        <div className="rounded-[2rem] bg-gradient-to-br from-slate-900 to-slate-700 p-1 shadow-2xl shadow-slate-900/10 dark:shadow-black/50">
          <div className="rounded-[1.75rem] bg-white p-8 dark:bg-slate-950">
            <div className="flex items-center justify-between gap-4 rounded-3xl bg-slate-100 px-5 py-4 dark:bg-slate-900">
              <div>
                <p className="text-sm font-medium text-slate-600 dark:text-slate-300">{t("statusToday")}</p>
                <p className="text-2xl font-semibold text-slate-950 dark:text-white">{t("tasksReady")}</p>
              </div>
              <span className="inline-flex rounded-full bg-slate-800 px-3 py-1 text-xs font-semibold text-white">{t("activeLabel")}</span>
            </div>
            <div className="mt-6 space-y-4">
              {[
                { title: t("task1Title"), tag: t("task1Tag"), meta: t("task1Meta") },
                { title: t("task2Title"), tag: t("task2Tag"), meta: t("task2Meta") },
                { title: t("task3Title"), tag: t("task3Tag"), meta: t("task3Meta") },
              ].map((task) => (
